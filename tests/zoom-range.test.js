...
test('production slider exposes the same broad range as camera bounds', () => {
  const index = read('index.html');
  assert.match(index, /id="zoomRange"[^>]*min="0\.18"[^>]*max="2\.4"[^>]*step="0\.005"/);
  assert.ok(MAX_ZOOM / MIN_ZOOM > 13);
  assert.ok(BASE_ZOOM > MIN_ZOOM && BASE_ZOOM < MAX_ZOOM);
});

test('touch zoom reaches both limits while preserving the gesture focal point', () => {
  const clientX = 220;
  const clientY = 410;
...
test('invalid touch zoom factor cannot corrupt the camera', () => {
