...
export class CardDetailController{
  constructor({root=document.body,onEdit,onDisplay}={}){
    ...
    ensureStyles();this.root=root;this.onEdit=typeof onEdit==='function'?onEdit:null;this.onDisplay=typeof onDisplay==='function'?onDisplay:null;this.activeCard=null;this.sourceElement=null;this.closeTimer=null;this.openToken=0;this.abortController=new AbortController();this.createOverlay();
  }

  createOverlay(){
    const overlay=document.createElement('div');overlay.className='card-detail-overlay';overlay.hidden=true;overlay.setAttribute('aria-hidden','true');overlay.innerHTML='<button class="card-detail-backdrop" type="button" aria-label="Закрыть карточку"></button><section class="card-detail-stage" role="dialog" aria-modal="true" aria-label="Полная карточка"><button class="card-detail-close" type="button" aria-label="Закрыть">×</button><div class="card-detail-content"></div></section>';this.root.append(overlay);this.overlay=overlay;this.stage=overlay.querySelector('.card-detail-stage');this.content=overlay.querySelector('.card-detail-content');this.closeButton=overlay.querySelector('.card-detail-close');
    const signal=this.abortController.signal;overlay.querySelector('.card-detail-backdrop').addEventListener('click',()=>this.close(),{signal});this.closeButton.addEventListener('click',()=>this.close(),{signal});document.addEventListener('keydown',(event)=>{if(event.key==='Escape'&&this.isOpen())this.close();},{signal});
  }

  isOpen(){return Boolean(this.activeCard&&!this.overlay.hidden);}
  ...
  open(card,source){
    if(!card||!(source instanceof Element))return false;clearTimeout(this.closeTimer);this.openToken+=1;const token=this.openToken;this.activeCard=card;this.sourceElement=source;this.content.replaceChildren(prepareClone(card,source));this.overlay.hidden=false;this.overlay.setAttribute('aria-hidden','false');this.overlay.classList.remove('is-visible');document.documentElement.classList.add('card-detail-active');this.setTransform(source.getBoundingClientRect());
    this.content.querySelector('[data-detail-edit]').addEventListener('click',()=>this.runAction(this.onEdit));this.content.querySelector('[data-detail-display]').addEventListener('click',()=>this.runAction(this.onDisplay));requestAnimationFrame(()=>{if(token!==this.openToken||this.overlay.hidden)return;this.overlay.classList.add('is-visible');this.closeButton.focus({preventScroll:true});});return true;
  }

  runAction(handler){const card=this.activeCard;if(!card||!handler)return;this.close({immediate:true});handler(card);}

  close({immediate=false}={}){
    clearTimeout(this.closeTimer);document.documentElement.classList.remove('card-detail-active');if(!this.isOpen())return;this.openToken+=1;this.setTransform(this.getSourceRect());this.overlay.classList.remove('is-visible');const finish=()=>{this.overlay.hidden=true;this.overlay.setAttribute('aria-hidden','true');this.content.replaceChildren();this.activeCard=null;this.sourceElement=null;document.documentElement.classList.remove('card-detail-active');};if(immediate||window.matchMedia?.('(prefers-reduced-motion: reduce)').matches){finish();return;}this.closeTimer=setTimeout(finish,TRANSITION_MS);
  }

  destroy(){clearTimeout(this.closeTimer);this.close({immediate:true});this.abortController.abort();this.overlay.remove();}
}
