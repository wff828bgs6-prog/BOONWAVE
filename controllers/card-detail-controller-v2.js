...
const STATUS_LABELS = Object.freeze({
  ...
  in_progress: 'В работе', paused: 'На паузе', completed: 'Завершено', cancelled: 'Отменено',
});
...
function countActive(items = []) { return items.filter((item) => (item.lifecycleStatus ?? 'active') === 'active').length; }
function activeTasks(tasks = []) { return tasks.filter((task) => (task.lifecycleStatus ?? 'active') === 'active' && task.status !== 'completed'); }
