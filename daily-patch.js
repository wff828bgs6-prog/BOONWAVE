(()=>{
  ...
  function expensesByStageHtml(n){
    const stages=n.stages&&n.stages.length?n.stages:[{id:'',title:'Без этапа'}];
    return stages.map(s=>{
      const list=(n.expenses||[]).filter(x=>(x.stageId||'')===s.id);
      const total=list.reduce((a,x)=>a+Number(x.amount||0),0);
      return '<section class="task-archive-group"><header><div><small>ЭТАП</small><h3>'+esc(s.title)+'</h3></div><span>'+money(total)+'</span></header><div class="task-archive-list">'+(list.length?list.map(x=>'<article class="task-archive-item expanded"><button type="button" class="task-archive-item-main"><div><b>'+esc(x.title||'Затрата')+'</b><small>'+esc(x.date||'')+'</small></div><strong>'+money(x.amount)+'</strong></button></article>').join(''):'<div class="task-archive-empty">Затрат нет.</div>')+'</div></section>'
    }).join('')
  }
  ...
  safe(()=>{expenseListHtml=function(n){
    const sid=state.selectedProcessStageId,first=(n.stages||[])[0]?.id||'';
    const list=(n.expenses||[]).filter(x=>(x.stageId||first)===sid);
    if(!list.length)return '<div class="note-block">Затраты этапа «'+esc(stageTitle(n,sid))+'» ещё не добавлены.</div>';
    return list.slice().reverse().map(x=>{
      const selected=state.selectedExpenseIds.has(x.id);
      return '<label class="expense-item '+(state.expenseSelectionMode?'selecting ':'')+(selected?'selected':'')+'" data-expense-id="'+esc(x.id)+'">'+(state.expenseSelectionMode?'<input type="checkbox" data-expense-select="'+esc(x.id)+'" '+(selected?'checked':'')+'>':'')+'<div><b>'+esc(x.title)+'</b><small>'+esc(stageTitle(n,x.stageId))+' · '+esc(x.date||'')+'</small></div><strong>'+money(x.amount)+'</strong></label>'
    }).join('')
  }});
