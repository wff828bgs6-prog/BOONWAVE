...
class BoonwaveDatabase {
  ...
  initDB() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => runSchemaMigrations(event.target.result, event.oldVersion);
      ...
      request.onblocked = () => console.warn('IndexedDB upgrade is blocked by another open BOONWAVE tab.');
    });
  }
  ...
  async deleteCardWithLinks(cardId, linkIds = []) {
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.onabort = () => reject(
        transaction.error ?? new Error('IndexedDB card transaction aborted.'),
      );
    });
  }
  ...
  async saveMedia(record, blob) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (blob !== undefined) transaction.objectStore('mediaBlobs').put({ id: record.id, blob });
  ...
  async loadAllData() {
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.oncomplete = () => {
        const cardsObj = {};
        for (const card of cardsRequest.result ?? []) cardsObj[card.id] = card;
