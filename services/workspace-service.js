...
import { resetLegacyCardLayer } from './card-layer-reset-service.js';
import { BASE_ZOOM, clampZoom } from '../canvas/camera.js';

export const LEGACY_CARD_LAYER_RESET_ACTIVE = true;
...
export async function loadWorkspace(options = {}) {
  ...
  const resetCardLayer = options.resetCardLayer ?? LEGACY_CARD_LAYER_RESET_ACTIVE;
  ...
  if (resetCardLayer) {
    await resetLegacyCardLayer({ stateStore, storageAdapter });
  } else {
